...
function setupEventListeners() {
  ...
  // Custom dimensions
  elements.customWidth.addEventListener('input', updatePreview);
  elements.customHeight.addEventListener('input', updatePreview);
...
function handleBannerPresetChange() {
  ...
  if (selectedPreset === 'CUSTOM') {
    ...
    elements.bannerName.value = '25_000_CUSTOM_HA_Codev';
  } else {
...
function updatePreview() {
  ...
  if (selectedPreset === 'CUSTOM') {
    width = parseInt(elements.customWidth.value) || 640;
    height = parseInt(elements.customHeight.value) || 360;
    presetName = 'Custom';
  } else if (selectedPreset && BANNER_PRESETS[selectedPreset]) {
  ...
  // Calculate preview dimensions (max 300px width for mobile)
  const maxWidth = window.innerWidth < 768 ? 250 : 300;
  const maxHeight = 200;
...
function toggleAdvancedSettings() {
  ...
  if (isVisible) {
    ...
    elements.toggleAdvanced.querySelector('.toggle-text').textContent = 'Show Advanced';
  } else {
    elements.advancedSettings.style.display = 'block';
    elements.toggleAdvanced.classList.add('active');
    elements.toggleAdvanced.querySelector('.toggle-text').textContent = 'Hide Advanced';
  }
}
...
function getCurrentConfig() {
  ...
  if (selectedPreset === 'CUSTOM') {
    width = parseInt(elements.customWidth.value) || 0;
    height = parseInt(elements.customHeight.value) || 0;
  } else if (selectedPreset && BANNER_PRESETS[selectedPreset]) {
    const preset = BANNER_PRESETS[selectedPreset];
    width = preset.width;
    height = preset.height;
