...
function setupEventListeners() {
  ...
  // Basic settings
  elements.bannerName.addEventListener('input', () => {
    markInputAsTouched(elements.bannerName);
    validateForm();
  });
  elements.clickTagUrl.addEventListener('input', () => {
    markInputAsTouched(elements.clickTagUrl);
    validateForm();
  });
  elements.description.addEventListener('input', () => {
    markInputAsTouched(elements.description);
    validateForm();
  });
  ...
  elements.canvasWidth.addEventListener('input', () => {
    markInputAsTouched(elements.canvasWidth);
    validateForm();
  });
  elements.canvasHeight.addEventListener('input', () => {
    markInputAsTouched(elements.canvasHeight);
    validateForm();
  });
  elements.canvasLeft.addEventListener('input', () => {
    markInputAsTouched(elements.canvasLeft);
    validateForm();
  });
  elements.canvasTop.addEventListener('input', () => {
    markInputAsTouched(elements.canvasTop);
    validateForm();
  });
...
function handleBannerPresetChange() {
  ...
  // Mark banner preset as touched
  markInputAsTouched(elements.bannerPreset);

  if (selectedPreset) {
    elements.bannerName.value = `25_000_${selectedPreset}_HA_Codev`;
    // Mark banner name as touched since we auto-populated it
    markInputAsTouched(elements.bannerName);

    // Auto-populate canvas dimensions with banner dimensions if fields are empty
    const preset = BANNER_PRESETS[selectedPreset];
    if (preset) {
      if (!elements.canvasWidth.value) {
        elements.canvasWidth.value = preset.width;
        markInputAsTouched(elements.canvasWidth);
      }
      if (!elements.canvasHeight.value) {
...
function updateConfigSummary() {
  ...
  const summaryItems = [
    ...
    { label: 'Canvas Size', value: `${config.canvasWidth}×${config.canvasHeight}px` },
    { label: 'Canvas Position', value: `${config.canvasLeft}, ${config.canvasTop}px` },
...
function validateForm() {
  const config = getCurrentConfig();
  const isValid = config.bannerName && config.clickTagUrl && config.width && config.height && config.width > 0 && config.height > 0 && config.canvasWidth > 0 && config.canvasHeight > 0 && config.canvasLeft !== null && config.canvasLeft !== undefined && config.canvasTop !== null && config.canvasTop !== undefined;
...
function getCurrentConfig() {
  ...
  if (selectedPreset && BANNER_PRESETS[selectedPreset]) {
    const preset = BANNER_PRESETS[selectedPreset];
    bannerWidth = preset.width;
    bannerHeight = preset.height;
  } else {
    bannerWidth = 0;
    bannerHeight = 0;
  }

  // Get canvas configuration from form inputs
  // Use banner dimensions as default for canvas size if not specified
  const canvasWidth = parseInt(elements.canvasWidth.value) || bannerWidth;
  const canvasHeight = parseInt(elements.canvasHeight.value) || bannerHeight;
  const canvasLeft = parseInt(elements.canvasLeft.value) || DEFAULTS.canvasLeft;
  const canvasTop = parseInt(elements.canvasTop.value) || DEFAULTS.canvasTop;
...
function replacePlaceholders(template, config) {
  return template
    ...
    .replace(/{{CANVAS_WIDTH}}/g, config.canvasWidth)
    .replace(/{{CANVAS_HEIGHT}}/g, config.canvasHeight)
...
function closeSuccessModal() {
  ...
  // Reset canvas values (empty so they auto-populate from banner dimensions)
  elements.canvasWidth.value = '';
  elements.canvasHeight.value = '';
  elements.canvasLeft.value = DEFAULTS.canvasLeft;
  elements.canvasTop.value = DEFAULTS.canvasTop;

  // Clear all touched states and validation classes
  const inputs = document.querySelectorAll('.form-input, .form-select');
  inputs.forEach((input) => {
...
function updateValidationStates() {
  ...
  inputs.forEach((input) => {
    ...
    // Only apply validation styling if the input has been interacted with
    if (input.dataset.touched === 'true') {
      // Special handling for canvas size fields - empty is valid (uses banner dimensions)
      const isCanvasSizeField = input.id === 'canvasWidth' || input.id === 'canvasHeight';

      if (isCanvasSizeField) {
        // Canvas size fields are valid if empty (auto-filled) or positive number
        if (input.value.trim() === '' || (input.checkValidity() && parseInt(input.value) > 0)) {
        ...
        if (input.checkValidity() && input.value.trim() !== '') {
          input.classList.add('validation-valid');
        } else if (!input.checkValidity() || (input.required && input.value.trim() === '')) {
          input.classList.add('validation-invalid');
        }
      }
    }
